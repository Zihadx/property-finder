...
const rows: { label: string; render: (p: Property) => React.ReactNode }[] = [
  {
    label: "Price",
    render: (p) => (
      <PropertyPrice price={p.price} purpose={p.purpose} size="sm" />
    ),
  },
  ...
  {
    label: "Area",
    render: (p) => `${p.areaSqft.toLocaleString("en-BD")} sqft`,
  },
  {
    label: "Price / sqft",
    render: (p) => formatBDT(Math.round(p.price / p.areaSqft)),
  },
  { label: "Parking", render: (p) => p.parking },
  { label: "Status", render: (p) => p.status },
  {
    label: "Amenities",
    render: (p) => p.amenities.slice(0, 4).join(", ") || "—",
  },
];

/**
 * Client-only: driven entirely by browser Redux state (which properties
 * are in the compare list). Split out from the page so SiteHeader/
 * SiteFooter in the parent can stay server-rendered — SiteFooter fetches
 * area data and can't render inside a "use client" tree.
 */
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.label}>
                    <th
                      scope="row"
                      className="ledger-label border-b border-border p-3 text-left align-top"
                    >
                      {row.label}
                    </th>
                    {properties.map((p) => (
                      <td
                        key={p.id}
                        className="border-b border-border p-3 align-top text-sm text-foreground"
                      >
                        {row.render(p)}
                      </td>
                    ))}
